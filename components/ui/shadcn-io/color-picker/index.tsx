...
export const ColorPicker = ({
  ...
  const [hue, setHue] = useState(defaultColor.hue() || 0);
  const [saturation, setSaturation] = useState(defaultColor.saturationl() || 100);
  const [lightness, setLightness] = useState(defaultColor.lightness() || 50);
  const [alpha, setAlpha] = useState(defaultColor.alpha() * 100);
  ...
  // Update color when controlled value changes (from external source only)
  useEffect(() => {
  ...
  // Notify parent of changes (only when not from external update)
  useEffect(() => {
