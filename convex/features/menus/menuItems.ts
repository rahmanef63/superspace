...
export const syncMenuMappings = action({
  ...
  handler: async (ctx, args) => {
    ...
    const items = await runQuery((api as any)["features/menus/menuItems"].getWorkspaceMenuItems, { workspaceId: args.workspaceId })
    for (const item of items as any[]) {
      if (item?.component && !known.has(item.component)) {
        ...
        if (def?.component) {
          await runMutation((api as any)["features/menus/menuItems"].setMenuItemComponent, {
            workspaceId: args.workspaceId,
            slug: item.slug,
