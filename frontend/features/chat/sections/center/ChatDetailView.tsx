import { useMobileWhatsApp, useCurrentChat } from '../../shared/hooks';
...
export function ChatDetailView({
  ...
  const liveMessages: Message[] | undefined = useMemo(() => {
    ...
    return (live as any[]).map((m: any): Message => {
      ...
      const fileSize = (m?.metadata as any)?.fileSize;
      return {
        id: String(m?._id || ''),
        ...
        mediaUrl: (m as any)?.attachmentUrl || undefined,
        fileName: (m?.metadata as any)?.fileName,
        fileSize: typeof fileSize === "number" ? String(fileSize) : fileSize,
  ...
  // Resolve target member id (other participant) for contact info
  ...
  const chatContact = useMemo<MemberInfoContact | undefined>(() => {
    if (!selectedChat || !contactMemberId) return undefined;
    return {
      id: contactMemberId,
      name: selectedChat.name || "Unknown contact",
