...
import type { ConversationItem } from "@/frontend/shared/communications/conversation"
...
export function ChatListView({ showArchived = false, variant = "standalone" }: ChatListViewProps) {
  ...
  // CRUD Handlers
  const handleEditChat = useCallback((item: ConversationItem) => {
    ...
    <div className={containerClasses}>
      ...
      {!showArchived && (
        <CreateConversationModal
          ...
          onCreateDirectChat={async (ContactId: Id<"users">, contactInfo?: { name?: string; imageUrl?: string }) => {
            try {
              let convId: Id<"conversations">
              if (globalMode) {
                convId = await createGlobalDirect({ otherUserId: ContactId })
              } else if (workspaceId) {
              ...
              // Optimistically add the chat to the store so it can be found immediately
              const meId = String((me as any)?._id || "")
              addChat({
                id: String(convId),
                name: contactInfo?.name || "Direct Chat",
                lastMessage: "",
                ...
                participants: [meId, String(ContactId)],
                tags: ['Direct', globalMode ? 'Global' : 'Workspace', 'Contacts'],
              })
              ...
              // Reload chats in background to get full data
              loadChats()
            } catch (e) {
          ...
          onCreateGroupChat={async (name: string, participantIds: Id<"users">[]) => {
            try {
              ...
              addChat({
                ...
                unreadCount: 0,
                isGroup: true,
                avatar: "", // Default group avatar logic usually handled by UI if empty
                participants: participantIds ? participantIds.map(String) : [],
              })
